...
class Game:
    ...
    def __init_fire(self) -> None:
        if self.__spaceship_sprite.sprite is not None:
            fire = Fire(
                pygame.Vector2(self.__spaceship_sprite.sprite.rect.centerx - FIRES["side"] // 2, self.__spaceship_sprite.sprite.rect.y + FIRES["side"]),
                FIRES["side"],
                FIRES["speed"].copy()
            )
            FireSprite(fire, pygame.Color(pygame.color.THECOLORS["white"]), self.__fires_sprites, self.__borders_collision_sprites)
